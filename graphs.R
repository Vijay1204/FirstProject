...
women
plot(women)
plot(women, type='p', pch=3,col='purple')#pch is used to mention the style in which the graph plot shopuld look from the default list
plot(women, type='l')
plot(women, type='b')
...
plot(women)
abline(lm(women$weight ~ women$height), col='red', lty=2, lwd=4)#lty is line type
stem(women$height)
...
table(gender)
pie(table(gender))
x = c(10,20,40,50) #add a pie or barchart
pie(x)
xlabels = c('A','B','C','D')
